...
Valid Username
    ...
    Textfield Value Should Be           id=user-name                    ${VALID_USER}
    #! ถ้าใช้อันนี้ ไม่ได้ เพราะสมมุติกรอกแค่ user มันมีส่วนประกอบ user อยู่ใน ${VALID_USER} มันก็จะผ่าน ให้ใช่้ Element attribute Value Should Be หรือ Textfield Value Should Be

Input Password
    #step 1.2: Enter valid data in Password text field 
    Input Text                          id=password                     ${PASSWORD}
    #step 1.2.1: Varify Password have a value
    Textfield Value Should Be           id=password                     ${PASSWORD}
...
Add Sauce Labs Backpack 1 ea
    ...
    Element Text Should Be              //div[text()='Sauce Labs Backpack']                                 Sauce Labs Backpack  #! Fix:   use Wait Until Page Contains Element
    ...
    Set Test Variable                  ${shopping_Cart_Start}                                              ${shopping_Cart}   
